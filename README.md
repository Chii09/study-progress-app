...
- **教材の編集・削除** — 登録済みの教材名・全体量を編集、不要になった教材を削除できる（削除時は紐づく進捗記録も連動して削除）
...
- **入力バリデーション** — 教材名・全体量・進捗量などを検証し、不正な値の登録を防止
...
- **2 テーブル構成による設計** — 教材と進捗記録を分離し、1 つの教材に複数の進捗記録が紐づく構造にした。教材削除時は外部キー制約（`ON DELETE CASCADE`）で関連する進捗記録も自動的に削除されるようにしている。
...
- **セキュリティ対策** — 出力値はすべて `htmlspecialchars` でエスケープし XSS を防止。SQL はプレースホルダを用いたバインドで発行し SQL インジェクションを防止。PDO のエラーモードを例外送出（`ERRMODE_EXCEPTION`）にして異常時に気づけるようにした。
- **サーバーサイドバリデーション** — 教材名の文字数、全体量・進捗量の数値範囲、教材 ID の存在チェックなどをサーバー側で検証し、不正な値の登録を防止。
- **PRG（Post/Redirect/Get）パターン** — フォーム送信後は `header('Location: ...')` でリダイレクトし、ブラウザの再読み込みによる二重送信を防止。エラー・成功メッセージはセッションを使ったフラッシュメッセージとして表示。
...
| `m6_edit.php` | 教材編集画面。教材名・全体量を更新 |
| `m6_delete.php` | 教材削除処理（POST 専用）。関連する進捗記録も連動して削除 |
...
- CSRF トークンの導入や、削除の確認画面をサーバーサイドでも用意するなど、堅牢性をさらに高める
